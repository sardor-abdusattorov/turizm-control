@php
    ...
    $colorFor = function (ContractApproverStatus $status): array {
        return match ($status) {
            ...
            ContractApprover::STATUS_QUEUED => ['bg' => 'rgba(37,99,235,.10)', 'fg' => '#1d4ed8', 'dot' => '#93c5fd'],
    ...
    $avatarOf = fn (ContractApprover $a): string => $a->user?->getFilamentAvatarUrl()
        ?? 'https://ui-avatars.com/api/?name='.urlencode($a->user?->name ?? '?').'&color=60A5FA&background=DBEAFE';
@endphp

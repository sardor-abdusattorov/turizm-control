...
<div>
    @if ($participations->isEmpty())
        <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ __('app.message.no_projects_for_sponsor') }}
        </p>
    @else
        {{-- One row per participation: project + its dates on the left, pledged
             and paid on the right. Tailwind utilities + inline flex only, since
                ...
                <div class="border-t border-gray-100 text-gray-700 first:border-t-0 dark:border-white/5 dark:text-gray-200"
                    ...
                    <div style="flex:1 1 auto;min-width:0;">
                        ...
                        <div class="text-gray-400" style="font-size:.72rem;margin-top:.1rem;">
                            {{ $p->project?->starts_on?->format('d.m.Y') ?? '—' }}
                        </div>
                    </div>

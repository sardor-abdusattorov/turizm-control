...
<div>
    ...
    @else
        {{-- One row per participant: name + payment state on the left, pledged
             and paid on the right — the same record-list layout as the sponsor
             and contact breakdowns. Inline flex only, since this renders inside
             a modal outside the .pj token scope. --}}
